...
Filter by evalue (evalue< 1e-50), ct4 (ct4>0.25), ce (ce>1.5), BitDiffSum (BitDiffSum>100) and the length of the candidate (length>100)

```{r}
dfFilter<-subset(df,log.besteval>50 & ct4>0.25 & ce>1.5 & BitDiffSum>100 & cand.end-cand.start>100)
```
Filtering by the above criteria reduces the number of candidates to 1959 candidates (originally: 5218) in all 50 genomes.


Remove anything at the beginning of a scaffold and loci over 50 kb.
...
```{r}
dfFilter2<-subset(dfFilter,locus.start > 1000)
```
Filtering by the above criteria reduces the number of candidates further to 1302 candidates (originally: 5218) in all 50 genomes.
