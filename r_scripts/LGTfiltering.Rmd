...
# extract eval
1. Extract the evalue of the best prokaryotic hit (take column "bestProHit". In this column, the eval is located  
...
```
Filtering by the above criteria reduces the number of candidates to 1978 candidates (originally: 5218) in all 50 genomes.


What if we remove anything at the beginning of a scaffold and loci over 50 kb?
...
```
Filtering by the above criteria reduces the number of candidates further to 1909 candidates (originally: 5218) in all 50 genomes.
...
write.table(dfC.filtered,"/Users/Janina/GAGA.LGT.filtered.tsv",sep="\t",quote = F,row.names = F)
