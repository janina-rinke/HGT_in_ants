...
```
This further reduces the number of candidates to 1716 possible LGTs (originally: 5218) in all 50 genomes.
...
write.table(dfC.filtered,"/Users/Janina/sciebo/Master.LGTs/GAGA.LGT.50.filtered.tsv",sep="\t",quote = F,row.names = F)
...
```
Merging candidates in close proximity together and filtering by the above criteria to only obtain candidates which are also in the dfFilter3 dataframe reduces the number of candidates further to 250 candidates (originally: 5218) in all 50 genomes.
