...
for (i in seq_along(dataFiles3)) {
  file<-NULL
  file<-merge(dataFiles3[[i]],gffs[[i]],by.x=c("locus.scaffold","CDS.start","CDS.end"),by.y=c("V1","V4","V5"),all.x=T,all.y=T)
  
dataFiles4[[i]]<-file
}

names(dataFiles4)<-gsub("/Users/Janina/sciebo/Master.LGTs/dfast/(.*?)\\.fa.*","\\1",gff.files.to.load)
```
