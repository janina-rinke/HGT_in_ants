...
```{r}
tsv2load<-"/global/scratch2/j_rink02/master/lgt/0_data/*/results/*.*.lgt.good.candidates.tsv" #provides the path to all files matching the pattern
...
```
The list "dataFiles" contains `r length(dataFiles)` elements from 163 genomes with LGT candidates predicted either by a euk database or a noAnt database. Each genome can have one noAnt candidate table and one euk candidate table, which means out of possible 326 (163 genomes x 2 database predictions) elements, only 7 elements were missing from the genomes and almost all GAGA genomes had both database LGT prediction tables.
...
```
The unfiltered df containes `r length(df)` candidates from all 162 selected genomes together.
...
ggsave("LGT_filtering.pdf",path="/Users/Janina/Documents/MASTER/Masterarbeit/plots/Thesis_Plots", width=10,height=8)
```
...
# Save the dataframe to fill in missing species names 
write_xlsx(df.new,"/home/j/j_rink02/Documents/Numbers_of_LGT_candidates_per_species.xlsx")

# Reload the table
LGT_candidates<-read_xlsx("/Users/Janina/Documents/MASTER/Masterarbeit/lgt/0_data/Numbers_of_LGT_candidates_per_species.xlsx")
...
ggsave("/Users/Janina/Documents/MASTER/Masterarbeit/plots/self_produced/Candidates_good_vs_outfiltered.pdf",p1,width=8,height=15)
...
ggsave("/Users/Janina/Documents/MASTER/Masterarbeit/plots/self_produced/Nr_predicted_candidates.pdf",p0,width=10,height=15)
``` 
...
ggsave("/Users/Janina/Documents/MASTER/Masterarbeit/plots/Thesis_Plots/Nr_predicted_candidates_vs_filtered_RAW.pdf",p1, width=10, height=7)
```
