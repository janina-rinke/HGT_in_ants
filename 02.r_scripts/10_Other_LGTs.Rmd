...
This script evaluates all other LGTs which have not previously been identified as ankyrin-repeat related, ancient, or orthologous LGTs occurring in groups. 
All of these other LGTs are investigated for their occurrence in ant species based on the CDS sequences of LGT loci (1,053 CDS in 497 LGT loci), their expression, and full-length annotation in order to infer LGTs with putative functionality apart from phylogenetic re-occurrence across related taxa in our dataset.
...
CFA.LGTs<-LGTs %>%
  dplyr::filter(type=="CFA")     # Only keep LGTs of the catecory "other"

library("xlsx")
write.xlsx(CFA.LGTs,"/Users/Janina/sciebo/HGT_Manuscript/Data/CFA_Synthases/CFA_HQ_HGTs.xlsx")
```
...
head(other.LGTs)
```
Considering only uniquely occurring HGTs results in 61 unqiue CDS in 45 loci. 43 ant species from seven subfamilies contained at least one uniquely occurring LGT. Bacterial donors were most often Wolbachia (n=89), followed by Spiroplasma (n = 13), Sodalis (n=9), Cardinium (n=9), Blochmannia-like (n=6) and other bacteria.
